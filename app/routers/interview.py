...
async def send_message(
    ...
    if interview.completed_at:
        return JSONResponse(status_code=400, content={"error": "Interview already completed"})

    _ = interview_crud.get_interviewee_by_interview(db, interview.id)  # Verify exists
    study = invite.study
    ...
    history_for_ai = [{"role": msg.role, "content": msg.content} for msg in conversation_history]
    ...
    try:
        ...
        return JSONResponse(
            status_code=500,
            content={"status": "error", "message": f"Failed to generate response: {str(e)}"},
        )
