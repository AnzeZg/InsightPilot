...
def test_get_studies_by_user(db_session, test_user):
    ...
    study1 = study_crud.create_study(db_session, test_user.id, "Study 1", "Desc 1", "Consent 1")
    study2 = study_crud.create_study(db_session, test_user.id, "Study 2", "Desc 2", "Consent 2")
...
def test_get_studies_by_user_pagination(db_session, test_user):
    ...
    for i in range(5):
        study_crud.create_study(db_session, test_user.id, f"Study {i}", f"Desc {i}", "Consent")
...
def test_update_question_text(db_session, test_study):
    ...
    question = study_crud.create_study_question(db_session, test_study.id, "Original text", 0)
